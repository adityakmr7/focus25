...
service cloud.firestore {
  ...
  async createIndexes() {
    try {
      ...
      log(
        'You can deploy indexes using: firebase deploy --only firestore:indexes'
      );
  ...
  async verifySetup() {
    try {
      ...
      const statsSnapshot = await this.db
        ...
        .limit(1)
        .get();
      const themesSnapshot = await this.db
        .collection('users')
        .doc(userId)
        .collection('themes')
        .limit(1)
      ...
      const todosCount = (
        await this.db.collection('users').doc(userId).collection('todos').get()
      ).size;
      const statsCount = (
        await this.db
          .collection('users')
      ...
      const themesCount = (
        await this.db.collection('users').doc(userId).collection('themes').get()
      ).size;
  ...
  async run() {
    try {
      ...
      const steps = [
        ...
        {
          name: 'Setting up Security Rules',
          fn: () => this.createSecurityRules(),
        },
