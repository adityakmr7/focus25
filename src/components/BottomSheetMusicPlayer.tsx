...
export const BottomSheetMusicPlayer: React.FC<BottomSheetMusicPlayerProps> = ({
  ...
  const getFilteredTracks = () => {
    ...
    if (filteredType === 'favorites') {
      return musicTracks.filter(track =>
        settings.favoriteTrackIds.includes(track.id)
      );
    }
  ...
  const waveStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      { scale: interpolate(waveAnimation.value, [0, 1], [0.8, 1.2]) },
    ],
  }));
  ...
  const renderTrackItem = ({ item }: { item: MusicTrack }) => {
    ...
    const isCurrentlyLoading =
      isSelected && downloadProgress > 0 && downloadProgress < 1;
    ...
    return (
      ...
      >
        <View
          style={[styles.trackIcon, { backgroundColor: item.color + '20' }]}
        >
          {isCurrentlyLoading ? (
