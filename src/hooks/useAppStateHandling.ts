  ...
  useEffect(() => {
    ...
    const handleAppStateChange = async (nextAppState: AppStateStatus) => {
      if (nextAppState === 'background') {
        console.log(
          '📱 App backgrounded - Live Activity background updates enabled'
        );
        ...
        if (timer.isRunning && !timer.isPaused) {
          console.log(
            '⏰ Timer is active - native background updates will continue'
          );
        }
      } else if (nextAppState === 'active') {
        console.log(
          '📱 App foregrounded - synchronizing with native timer state'
        );
