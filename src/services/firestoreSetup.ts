...
export class FirestoreSetup {
   ...
   */
  static async testFirestoreConnection(): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
   ...
   */
  static async createSampleUserProfile(
    force: boolean = false
   ...
   */
  static async createSampleUserData(
    force: boolean = false
  ): Promise<{ success: boolean; message: string }> {
    try {
      const user = auth().currentUser;
      ...
      const userDataRef = firestore()
        .collection(FIRESTORE_COLLECTIONS.USER_DATA)
        .doc(user.uid);
   ...
   */
  static async testSecurityRules(): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
  ...
  static async runCompleteSetup(): Promise<void> {
    ...
    console.log(
      '3. Try "Backup to Cloud" and "Restore from Cloud" in Settings'
    );
  }
