...
export class NotificationService {
  ...
  async scheduleSessionComplete(isBreak: boolean = false): Promise<void> {
    if (!this.settings.enabled || (!this.settings.sessionComplete && !isBreak) || (!this.settings.breakComplete && isBreak)) {
      return;
    }
    ...
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: isBreak 
          ...
          : this.settings.motivationalMessages ? randomMessage : 'Time for a well-deserved break!',
  ...
  async toggleNotificationType(type: keyof NotificationSettings, enabled: boolean): Promise<void> {
