  ...
  async updateTodo(
    ...
    try {
      ...
      if (updates.createdAt) {
        updateData.createdAt = firestore.Timestamp.fromDate(
          new Date(updates.createdAt)
        );
      }
      if (updates.completedAt) {
        updateData.completedAt = firestore.Timestamp.fromDate(
      ...
      if (filters.dateRange) {
        query = query
          .where(
            'createdAt',
            '>=',
            firestore.Timestamp.fromDate(filters.dateRange.start)
          )
          .where(
            'createdAt',
            '<=',
            firestore.Timestamp.fromDate(filters.dateRange.end)
          );
      }
