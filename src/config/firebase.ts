...
export const signInWithGoogle = async () => {
    try {
        ...
        try {
            ...
            console.warn('User profile creation failed, but sign-in succeeded:', profileError);
...
const createOrUpdateUserProfile = async (user: any, provider: 'google' | 'apple', customDisplayName?: string, retryCount: number = 0) => {
    const maxRetries = 3;
    ...
    try {
        ...
        try {
            ...
            console.log('Document check failed, attempting to create new user:', firestoreError.message);
        ...
        console.error(`Error creating/updating user profile (attempt ${retryCount + 1}):`, error);
        
        // Retry if it's a permission or not-found error and we haven't exceeded max retries
        if (retryCount < maxRetries && (error.code === 'firestore/not-found' || error.code === 'firestore/permission-denied')) {
            console.log(`Retrying user profile creation in ${(retryCount + 1) * 1000}ms...`);
            await new Promise(resolve => setTimeout(resolve, (retryCount + 1) * 1000));
            return createOrUpdateUserProfile(user, provider, customDisplayName, retryCount + 1);
        }
