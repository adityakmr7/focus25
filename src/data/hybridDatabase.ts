  ...
  async getTodos(
    ...
    if (user?.uid && user?.isPro) {
      try {
        ...
        const syncedTodos = await firebaseTodoService.getTodos(
          user.uid,
          options
        );
      ...
      console.log(
        'User authenticated but not pro - returning local todos only'
      );
    } else {
  ...
  async updateTodo(id: string, updates: any): Promise<void> {
    ...
    if (user?.uid && user?.isPro) {
      try {
        ...
        console.log(
          'Todo update synced to Firebase subcollection for pro user'
        );
      } catch (error) {
  ...
  async deleteTodo(id: string): Promise<void> {
    ...
    if (user?.uid && user?.isPro) {
      try {
        ...
        console.log(
          'Todo deletion synced to Firebase subcollection for pro user'
        );
      } catch (error) {
