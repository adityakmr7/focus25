...
const StatisticsScreen: React.FC<StatisticsScreenProps> = ({ navigation }) => {
  ...
  const { flowMetrics, initializeStore: initializePomodoro } =
    usePomodoroStore();
  ...
  const focusEfficiency =
    flows.completed > 0 ? Math.max(0, 100 - interruptions * 10) : 100;
  ...
  const statsData = [
    ...
    },
    {
      ...
      gradient:
        focusEfficiency >= 80 ? ['#10B981', '#34D399'] : ['#F093FB', '#F5576C'],
