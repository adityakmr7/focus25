...
import {
  ...
  canUseCloudSync,
  proFeatureService,
} from '../services/proFeatureService';
...
const SettingsScreenContent: React.FC<SettingsScreenProps> = ({
  navigation,
}) => {
  ...
  const handleCheckForUpdates = useCallback(async (): Promise<void> => {
    if (Platform.OS === 'web') {
      showAlert(
        'Not Available',
        'Update checking is not available on the web version.'
      );
      return;
    }
  ...
  const handleSyncFromCloud = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      showAlert(
        'Restore Failed',
        'Failed to restore data from cloud. Please try again.'
      );
    } finally {
      setIsSyncing(false);
    }
              ...
              <Stack direction='column' gap='md'>
                <View style={styles.timeDurationContainer}>
                  <View style={styles.durationContent}>
                    ...
                    />
                    <Text
                      style={[styles.timeDurationLabel, { color: theme.text }]}
                    >
                      Focus Duration
                    </Text>
                  </View>
            ...
            <CardContent>
              <Stack direction='column' gap='sm'>
                ...
                >
                  <DSSettingItem
                    title={
                      isSyncing ? 'Syncing to Cloud...' : 'Backup to Cloud'
                    }
                    subtitle={
                      isSyncing
                        ? 'Please wait...'
            ...
            <CardContent>
              <Stack direction='column' gap='sm'>
                ...
                <DSSettingItem
                  ...
                  onPress={
                    isCheckingUpdates ? undefined : handleCheckForUpdates
                  }
                />
                <DSSettingItem
                  title='Rate App'
