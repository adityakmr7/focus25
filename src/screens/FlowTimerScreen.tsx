...
const FlowTimerScreen: React.FC<FlowTimerScreenProps> = () => {
  ...
  // Music settings
  const { settings, loadSettings, saveSettings, handleVolumeChange } =
    useMusicSettings();
  ...
  const handleToggleTimer = useCallback(async () => {
    ...
    await handleTimerToggleWithBackground(
      wasRunning,
      wasPaused,
    ...
    async (delta: number) => {
      ...
      if (
        ...
        try {
          ...
          volumeAnimation.value = withTiming(updatedSettings.volume, {
            duration: 200,
          });
        } catch (error) {
